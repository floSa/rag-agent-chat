...
| P2 | `runs/reference.json` ne porte que 117 des 138 lignes | **Trouvé au lot 4.** 21 questions (G-118 à G-138) n'ont pas abouti lors de cette campagne, et `runs/README.md` l'annonçait pourtant à 138. C'était la cible de `make eval` : chaque exécution confrontait 138 moyennes à 117, en silence. La cible est passée à `runs/final.json` et la comparaison appariée refuse désormais ce couple, mais le fichier reste faux par rapport à son étiquette. Le rejouer demande la stack démarrée — et le jour où il le sera, `test_la_cible_historique_de_make_eval_serait_refusee` doit être retiré avec cette ligne. Débloqué par : la stack démarrée. |
