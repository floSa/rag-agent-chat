...
|---|---|---|
| **1** | modèle d'embedding `paraphrase-multilingual-MiniLM-L12-v2`, identique des deux côtés | OK **tenue et gardée des DEUX côtés** depuis le lot 3, fusionné le 7 septembre 2026 (`c5f9a54`). Le lecteur confronte son réglage à l'estampille de la collection **avant chaque recherche dense**, refuse aussi l'estampille absente, et rend **503** sans avoir chargé le moindre modèle. Quatre audits indépendants — §4.27
| **2** | `element_id` déterministe, 10 hexadécimaux | OK tenue par le pipeline, et l'agent le valide (`^[a-f0-9]{10}$`, `graph_context.py`). **Ce que ce dépôt en promettait était faux, et c'est corrigé** : `pour_le_pipeline_ingestion.md` écrivait que le déterminisme fait « survivre le jeu doré à une réingestion ». Il fait survivre un jeu à une réingestion **du même corpus** ; rien ne le fait survivre au **remplacement** d'un corpus, et aucune convention d'identifiant ne le pourrait. Corrigé au site et **gardé** contre son retour — §4.3 |
| **3** | `source_path` est l'identité d'un document | OK tenue |
| **4** | `sequence` porte l'ordre, monotone | OK tenue — et **reproduite de mes mains** : §4.5 |
| **5** | `POST /reindex` en fin de pipeline | OK **tenue et prouvée en marche** par le lot 1 — mesurée par le lot, puis **indépendamment par son audit sur l'agent vivant**, et gardée par un test que l'audit a mesuré seul garde de deux mutations du producteur. §4.2 |
...
|---|---|
| branches | `mesuré` le **4 septembre 2026, après la fusion du lot 2** : `main` = **`db05162`**, **14 commits d'avance sur `origin/main`** — non poussé. **Une seule** branche hors `main` : `<branche de session>`, l'arbre du pilote. Les **six** autres et leurs arbres ont été supprimés à la fusion, `.<branche de session>/` ne porte plus aucun répertoire mort, et les garde-fous ont été **réarmés puis éprouvés** — adresse interdite → `rc=1` et HEAD immobile, adresse autorisée au même nom → `rc=0`. Aucune branche distante autre que `main`. *Relevé antérieur, avant fusion : cinq branches hors `main`, deux errantes* — `<branche de session>` et `<branche de session>`, issues de sessions abandonnées, leurs arbres propres et sans travail à sauver. Les deux branches en vol du lot 2 : `<branche de session>` (`9435657`) et `<branche de session>` (`d5b2c3c`). Plus l'arbre du pilote, `<branche de session>`. **La règle « une branche par lot en vol » est en dette de nettoyage, et elle se paie à la fusion du lot 2** |
...
|---|---|---|
| **138 questions régénérées** sur le corpus en service | `tests/fixtures/golden_qa_generated.yaml` | OK **le jeu de RÉGLAGE**, cible de `make eval`. **130 / 130** ancrages présents dans ChromaDB **et** NebulaGraph, `mesuré` et versionné à `runs/2026-09-08-ancrages.json`. `reviewed: false` partout — c'est du **silver** |
| **30 questions**, écrites après l'ingestion | `tests/fixtures/jeu_de_questions_pipeline.yaml`, transposé de `documentation/campagnes/2026-09-02-jeu-de-questions.yaml` du pipeline, qui reste le **site canonique** | OK **le jeu de CONTRÔLE**, cible de `make eval-controle`. **44 / 44** ancrages présents dans les deux stores. Sa réserve voyage dans le fichier et un test refuse qu'elle en sorte |
| ~~138 questions générées le 3 août 2026~~ | ~~`tests/fixtures/golden_qa_generated.json`~~ | NON **RETIRÉ.** 0 / 129 ancrages dans le graphe, et **34** des 36 détections `detect-secrets` du dépôt |
| ~~15 questions écrites à la main~~ | ~~`tests/fixtures/golden_qa.json`~~ | NON **RETIRÉ, et il était le `--golden` par DÉFAUT.** **13** de ses 15 questions étaient à réponse — les deux autres, `Q-010` et `Q-011`, sont des abstentions, et « 15 à réponse » était faux de deux (`mesuré`, trouvaille N8) — et portaient **0** `gold_element_ids` : toutes ses métriques de rappel valaient `None`, ce qui se lit « sans objet » et non « cassé » — plus silencieux encore que le jeu de 138. Trouvé par un garde du lot 5 |
...
| | Le lot | État |
|---|---|---|
| **1** | armer le garde-fou d'identité (§4.1), puis démarrer l'agent et **prouver l'exigence 5** (§4.2) | OK **fusionné** `9596720` — livré (`Conv' 21`), audité (`Conv' 22`), réparé (`Conv' 23`), fusion tranchée par le pilote après vérification de ses deux gardes par mutation |
| **2** | les **trois réserves de lecture de `sequence`** (§4.5), et le garde qui les tient | OK **fusionné** `db05162` — livré (`Conv' 24`), audité (`Conv' 25`, 8 trouvailles dont 2 bloquantes), réparé (`Conv' 26`), **sa réparation auditée à son tour** (`Conv' 27`, 1 bloquante), réparée une seconde fois (`Conv' 28`). Fusion tranchée par le pilote après vérification des deux directions dangereuses du garde — §4.18. **Trois audits, trois trouvailles matérielles** |
| **3** | le garde du **modèle d'embedding** côté lecteur (§4.4) | OK **fusionné** `c5f9a54` — livré (`Conv' 29`), audité (`Conv' 30`, 2 bloquantes), réparé (`Conv' 31`), réaudité (`Conv' 32`, 1 bloquante), réparé (`Conv' 33`), audité en **étroit** sur la couche async (`Conv' 34`, 1 bloquante), réparé (`Conv' 35`). **Quatre audits, quatre trouvailles bloquantes, aucune régression fonctionnelle** — §4.27. Fusion tranchée par le pilote sous le **critère amendé** du §4.18 |
...
| **5** | **régénérer le jeu doré sur le corpus actuel ET adopter les 30 questions du pipeline** (§4.3), puis établir une **nouvelle campagne de référence** | OK **fusionné** `744c2c8` — livré (`Conv' 36`), audité (`Conv' 37`, **zéro bloquante**, 8 non bloquantes). **La mesure de qualité du dépôt était morte ; elle vit, et en deux instruments.** Le lot a publié deux lectures **contre lui-même**, et l'audit les a renforcées — §4.30 |
| **6** | le **garde du reranker** et les formes d'affectation qui échappaient au garde de sûreté (§4.31) | OK **fusionné** `137d780` — livré (`Conv' 40`), audité (`Conv' 41`, **2 bloquantes**), réparé (`Conv' 42`). **Le garde SIGNALE au lieu de refuser**, sur un chevauchement de vocabulaires reproduit deux fois, et le périmètre de l'inventaire a un plancher qui **ne descend jamais** — §4.32 à §4.35 |
| **7** | le garde de sûreté ne voit ni `setattr(settings, …)` (**47** occurrences — `git grep -c 'setattr(settings' -- '*.py'`, et **la portée est porteuse** : sans elle la même commande rend 52 dans 9 fichiers, en comptant les mentions en prose — §4.40) ni la lecture d'environnement à valeur par défaut (**6** sites suivis — le sixieme est un ALIAS, `env = os.environ.get`, invisible a un motif exigeant une parenthese collee) ; et la lecture défensive du reranker ne tient que dans **trois** natures d'exception sur sept sondées, **quatre** propageant et cassant la recherche (§4.35, mesuré au §4.37) | OK **fusionné** `71dfea8` — livré (`Conv' 43`), audité (`Conv' 44`, 1 bloquante), réparé (`Conv' 45`). **Ce dépôt a enfin un garde-fou sur `push`**, et la borne d'une ref neuve lit l'état RÉEL du distant — §4.36 à §4.40. Note historique : **moins cher qu'annoncé** : `mesuré`, **aucun** des 27 `setattr` sur `embedding_model_name` ne passe un littéral, ils passent tous une constante nommée, donc l'obstacle invoqué pour le différer **n'existe pas** |
| **8** | **armer l'agent en service, après avoir figé l'ancien lecteur** — né d'une trouvaille du lot 4 et non de ce plan : l'agent tournait le code du 3 septembre, et les gardes des lots 3 et 6 avaient **0 occurrence** dans le conteneur | OK **fusionné** `9509228` — livré (`Conv' 47`) sur décision de l'utilisateur. **Le garde du modèle d'embedding s'exécute enfin en production**, et le lecteur neuf rejoue les deux références du 8 septembre **à l'unité** : sept lots n'ont pas déplacé un caractère du contenu servi. La définition (C) du lot 4 survit derrière un réglage **éteint** — §4.42 et §4.43 |
...
| **84** | LOT-33 — la CI rougissait sur une course du MONTAGE | **LIVRÉ et FUSIONNÉ le 24 septembre 2026** `5fd2418`, **1125 passés**. `src/` N'EST PAS TOUCHÉ, vérifié — le §4.18 ne s'applique pas. **LE DIAGNOSTIC DU PILOTE ÉTAIT FAUX ET LE LOT L'A DÉMONTÉ PAR LA MESURE.** Je supposais que le drapeau n'avait pas encore été posé quand le test le lit. Un mouchard posé sur l'ensemble lui-même — journalisant chaque `add`/`discard` **avec l'identité du fil** — montre l'inverse : le drapeau **était posé, et par la boucle**, conformément au docstring ; il a été **effacé par le fil d'une sonde qu'un test ANTÉRIEUR avait abandonnée au plafond**, `_executer_sonde` retirant **par nom** et le nom étant partagé. `present=True` au moment du retrait : **il a effacé un drapeau vivant**. **ET LES TROIS `clear()` CENSÉS FERMER CE CAS L'AGGRAVAIENT** : ils orphelinent le fil encore vivant. Le correctif ne les remplace pas par un délai — il rend l'ensemble **OBSERVABLE** (`threading.Condition` sur `add`/`discard`/`clear`) et **ATTEND le dernier geste du fil**. Aucun `sleep`, aucune tolérance : le plafond de 10 s n'est **pas une marge dont dépend le résultat**, c'est le filet qui transforme un fil qui ne revient jamais en **rouge nommé** au lieu d'une suite figée. **REPRODUCTION AVANT RÉPARATION** — retard **injecté** sur le bord accusé, déclenché par l'entrée du fil suivant et **jamais par l'horloge** : **10 rouges sur 10**, au message exact de la CI. Après réparation, **même retard réinjecté : 0 sur 10**. **8 scènes sœurs relevées PAR MOTIF** — 6 dans `test_garde_modele_embedding.py`, 2 dans `test_health_parallele.py` — **aucune laissée découverte**, la barrière les couvre sans les modifier ; périmètre borné par `git grep` qui ne rend rien hors de `tests/unit/`. **MESURÉ PAR LE PILOTE SUR LE RÉSULTAT DE LA FUSION** : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1125** ; mutation du **producteur** — `attendre_le_vide` rendue immédiate, ancre vérifiée **unique avant d'écrire**, mutation **compilée** — → `rc(make)=2`, **1 rouge, exactement le garde** `test_la_barriere_ne_rend_la_main_qu_au_dernier_geste_du_fil` ; **restauration IDENTIQUE au SHA-256**, arbre propre. **ET LE PILOTE A RATÉ SA PREMIÈRE MUTATION SANS LE VOIR** : son script cherchait un `while` qui n'existe pas dans ce fichier, n'a donc **rien modifié**, et a rendu un **vert qu'il a failli lire comme « la garde ne mord pas »**. Le second essai **assert que le SHA-256 du fichier a changé** avant de mesurer. *Une mutation qui ne mute pas rend un vert qui ne prouve rien.* **QUATRE RÉSERVES DU LOT, TOUTES AU DÉPÔT §4.73**, dont deux qui comptent : **la reproduction SOUS CHARGE a ÉCHOUÉ** — 44 brouilleurs pour 22 cœurs, charge tenue, **0 rouge sur 8** — et *une charge qui ne reproduit pas ne disculpe rien* ; une tentative antérieure interrompue **mélangeait tours chargés et tours libres** et le lot l'a **écrite pour qu'on ne la reprenne pas**. Et **le run `35872551099` n'a pas été rejoué** : l'identité avec l'incident CI est une **inférence forte, pas une mesure**. **LE RÉSIDU DE PRODUCTION RESTE OUVERT ET N'EST PAS DE CE LOT** : `_executer_sonde` retire **par nom sans vérifier que le drapeau est le sien**. Le site le documentait comme « résidu assumé et borné » ; **ce lot prouve qu'il est réel et non théorique** — il vient de coûter un rouge de CI. La fermeture propre, un **jeton d'appartenance**, toucherait `src/` : non mesurée, non chiffrée |
...
| **85** | LOT-34 — la prise exclusive des sondes | **LIVRÉ et FUSIONNÉ le 24 septembre 2026** `0ef722e`, **1126 passés** (`tests/unit/`). Ferme le résidu que le §4.73 laissait ouvert : `_executer_sonde` retirait le drapeau **par nom** sans savoir s'il était le sien. **LE LOT A RÉPARÉ CONTRE LA LETTRE DE SON MANDAT, ET LE PILOTE L'A ACCEPTÉ** : le mandat disait « ne pas réparer si la mesure ne le justifie pas », et la mesure en service rendait **0 occasion en ~50 h** — mais **0 occasion n'est pas 0 coût**. La précondition est atteignable (démarrage du fil jusqu'à **13,8 s** sous contention du GIL, au-delà du plafond de 3 s), et le coût quand le cas se produit est mesuré sous retard injecté : **le doublement des sondes se transmet, 12 maillons sur 12**, tant que le store est lent. **UNE PRISE, PAS UN JETON** : `threading.Lock` neuf par appel, pris sans attente par le premier des deux, le fil à son entrée ou la boucle dans l'`except` ; le perdant ne fait RIEN. Un jeton aurait empêché le retrait, pas la sonde en double. Garde neuf : **10 rouges sur 10** sur la base, **10 verts sur 10** après. **MESURÉ PAR LE PILOTE SUR LE RÉSULTAT DE LA FUSION, CLONE PRINCIPAL, `.env` réel**, deux fois (sur `fef23da` à 14:06 UTC, puis sur la fusion définitive à 15:37 UTC) : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1126** ; trois mutations du **producteur**, sites relevés par motif avec unicité assertée, SHA-256 changé et `py_compile` vert — la boucle retire même perdante → **3 rouges** (la rafale, l'absorption, le second fil) ; la boucle ne retire jamais → **2 rouges et 2 erreurs**, dont le garde neuf ; le fil sonde même perdant → **1 rouge, le garde neuf** ; **témoin inerte** de 13 lignes → `rc=0`, **145** sur les quatre fichiers de sondes ; **restaurations IDENTIQUES au SHA-256**. **LE RÉSIDU N'A JAMAIS ÉTÉ VU SANS INJECTION**, ni par le lot ni par l'audit : la réparation ferme une fenêtre dont seules la précondition et la conséquence sont mesurées. **Non déployé à la fusion.** |
| **86** | AUDIT-34 — audit du lot 34 | **RIEN DE BLOQUANT, FUSIONNÉ le 24 septembre 2026** `33ace68`, rapport `documentation/audits/2026-09-24-audit-lot-34.md`. **Le §12 ferme le lot** : les sept non bloquantes vont au registre (§4.74) et n'appellent pas de réparation. Huit chemins de sortie joués sur les deux arbres : branche **8/8**, base **7/8**, le rouge étant la fenêtre ; la boucle ne se bloque pas (**0,0000 s** dans l'`except`, même quand le fil pend). Le `None` du perdant est rapporté dans un futur annulé et **jeté** par l'anyio installé (4.15.1), mouchard à l'appui ; **5** appelants en production, relevés par motif. **Occurrence naturelle sous une contention que le lot n'avait pas essayée** (`gc.collect()`, GIL tenu en C) : précondition reconfirmée (**10 497,8 ms**), fenêtre **non vue sur 250 essais**, contrôle positif base **100/100**, branche **0/100**. **L'AUDITEUR S'EST PRIS LUI-MÊME ET L'A ÉCRIT** : son premier détecteur lisait « drapeau absent » comme « la boucle l'a retiré », alors que le fil peut l'avoir retiré en revenant vite — il a annoncé **18 résidus sur la branche réparée**, puis un « 55 sur 500 » qu'il a **retiré** avant de rendre. *Un détecteur qui confond deux retireurs rend des chiffres faux dans les deux sens.* **Le constat E est le seul qui nomme une sortie fausse, et il est ANTÉRIEUR au lot** : `usage.py` fait un test-puis-usage sur `usage.sqlite-wal` ; non reproduit sur 4000 appels, mais quand la fenêtre tire, `/health` publie **0 interaction et 0 octet** pendant un tick pour une base qui en porte 500, et `failures` monte **définitivement**. |
| **—** | LE REDÉPLOIEMENT DE `main`, AVANT LA PURGE DU PIPELINE *(geste du PILOTE, aucune conversation)* | **Fait le 24 septembre 2026 à 15:44 UTC**, par la marche du §4 de `identite_du_code_servi.md`, depuis le clone principal. **Pourquoi maintenant** : le code servi, `ba6a8f0`, avait **45 commits de retard** sur `main`, et le pipeline va purger puis réingérer ses stores. Mesurer une réingestion sur un code servi en retard aurait mélangé deux changements. **(a)** l'image servie était `sha256:32d74a7d…` ; **(b)** étiquetée **`rag-agent-chat-agent-api:2026-09-24-avant-lot34`** AVANT toute construction, c'est le chemin du retour ; **(c)** l'étiquette pend bien à `32d74a7d…`, vérifié ; **(d)** `make image` → `rc=0`, `sha256:4739dd58…`, `RAG_AGENT_CODE_SHA=72522f3…` ; **(e)** `docker compose up -d --no-build agent-api` → `rc=0`, conteneur RECRÉÉ, `healthy` en **13 s** (`StartedAt=15:44:19Z`, `RestartCount=0`), `rag-frontend` non touché (`StartedAt` inchangé, 11:23:09Z) ; **(f)** `/health` publie `etat: identifie`, sha **`72522f3`**. **CE QUE LE DÉPLOIEMENT LIVRE, ÉPROUVÉ SUR LE SERVICE** : `/openapi.json` publie **`maximum: 10`** pour `max_sources` (au lieu de 20) ; `POST /answer` avec `max_sources=11` → **HTTP 422** ; une vraie question avec `max_sources=3` → **HTTP 200**, 1217 caractères, 5 citations, 13,7 s. Le lot 34 est servi. `index_lexical` était **froid** au démarrage (le piège paresseux connu, pas une panne) : `POST /reindex` → HTTP 200 en 0,41 s, puis les **quatre sondes vertes**. |
| **87** | LOT-35 — le `-wal` replié ne vide plus l'actif de `/health` | **LIVRÉ et FUSIONNÉ le 24 septembre 2026** `9fd5238`, **1128 passés** (`tests/unit/`). Ferme le constat E de l'audit 34 : dans `usage.stats()`, `annexe.exists()` puis `annexe.stat()` sur le `-wal`, que SQLite supprime à la fermeture du dernier lien. Quand la fenêtre tirait, `/health` publiait **0 interaction et 0 octet** pour une base pleine et `failures` montait définitivement. **La réparation rattrape `FileNotFoundError` sur le SEUL `stat()` de l'annexe** et garde l'`exists()` ; `PermissionError`, `IsADirectoryError` et toute autre `OSError` continuent de monter, justification écrite AU SITE. Garde **rouge d'abord** sous retard injecté, déclenché par un événement (le double de `Path.exists` supprime le `-wal` à l'instant où il vient de répondre vrai), la fenêtre étant assertée avant toute autre assertion. **LE LOT A ATTRAPÉ SA PROPRE JUSTIFICATION NON MESURÉE** : la mutation « rattrapage élargi à `OSError` » a d'abord **survécu à 1128 passés** — rien ne distinguait l'étroit du large —, puis le garde de borne ajouté pour elle est resté vert **pour une mauvaise raison**, son double refusant le PREMIER `stat()`, celui qu'`exists()` fait lui-même. Corrigé (refus au second appel, `assert len(appels) == 2`), la mutation meurt. **5 sœurs relevées par motif**, contrôle positif asserté ; les 4 autres partagent la forme mais pas le producteur de course, laissées avec leur réserve au §4.75. **AUDIT INDÉPENDANT NON DEMANDÉ, ET LA RAISON EST ÉCRITE ICI, comme à la ligne 83** : le §4.18 l'exige dès que `src/` change, mais le diff exécutable tient en **trois lignes** (`try`, `except FileNotFoundError`, `continue`), le lot a fourni sa table des mutations **dans les deux sens** (rattrapage retiré, rattrapage élargi en portée et en nature), et **le pilote les a remesurées de ses mains sur le RÉSULTAT DE LA FUSION**. **MESURÉ PAR LE PILOTE, clone principal, `.env` réel, 17:44 UTC** : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1128** ; mutation **rattrapage retiré** → `rc(pytest)=1`, **1 rouge, le garde neuf** ; mutation **`except OSError`** → `rc(pytest)=1`, **1 rouge**, `…une_annexe_illisible_reste_une_panne_et_se_compte` ; **témoin inerte** 13 lignes → `rc=0`, **27** sur `test_capture_usage.py` ; SHA-256 changé à chaque mutation, `py_compile` vert, **restaurations IDENTIQUES**. **LA FENÊTRE N'A JAMAIS ÉTÉ VUE SANS INJECTION** (0 sur 4000 appels pour l'audit). **Non déployé à la fusion.** |
| **—** | LE REDÉPLOIEMENT DU LOT 35 *(geste du PILOTE, aucune conversation)* | **Fait le 24 septembre 2026 à 17:49 UTC**, même marche du §4 de `identite_du_code_servi.md`. Image servie `sha256:4739dd58…` étiquetée **`rag-agent-chat-agent-api:2026-09-24-avant-lot35`** AVANT construction, étiquette vérifiée identique ; `make image` → `rc=0` ; `up -d --no-build agent-api` → `healthy` en 13 s, `RestartCount=0`, image `sha256:d296f489…` ; `/health` publie `etat: identifie`, sha **`13acdb1`** ; `POST /reindex` → 200, **quatre sondes vertes**, `usage.failures=0`. |
| **88** | LOT-36 — un jeu qui voit la sélection : ancrages multiples et dispersés | **LIVRÉ et FUSIONNÉ le 24 septembre 2026** `3cc6367`, **1160 passés** (`tests/unit/`). `src/` N'EST PAS TOUCHÉ, vérifié — le §4.18 ne s'applique pas. Ferme la dette ouverte à la ligne 82 : **le jeu de réglage était aveugle à la sélection**. `tests/fixtures/jeu_ancrages_disperses.yaml` : **60 questions, 120 ancrages (2 par question, dans deux sections distinctes), 109 distincts**, 109/109 dans le graphe et dans ChromaDB, `reviewed: false` sur les 60. La construction ne regarde **jamais** le retrieval (filtrer sur le rang mesuré rendrait l'instrument tautologique) : condition de reconstruction dans les deux sens, non-suffisance jugée passage par passage, gardes lexicaux. **L'INSTRUMENT VOIT k, et c'est la condition de recette** : questions dont TOUS les ancrages arrivent, de k=1 à k=10, **0, 2, 6, 7, 7, 7, 8, 8, 9, 10** — **+10 gagnées, 0 perdue** ; le même banc sur le jeu de réglage rend **+0 à toutes les bascules après k=2**, donc le plateau était une propriété du jeu et non du banc. **Recoupé par le pilote depuis les bascules versionnées** (`runs/2026-09-24-dispersion.json`) : somme des gagnées **10**, questions distinctes gagnées **10** = 10 − 0, perdues **0** ; éléments au prompt en moyenne **33,48** à k=3 et **102,5** à k=10. **CE QU'IL DIT DU RÉGLAGE, ET CE QU'IL NE DIT PAS** : de 3 à 6, **+1 question** ; de 3 à 10, **+4** ; les intervalles de Wilson se recouvrent. **`AUTO_SELECT_TOP_K` reste à 3.** **Le plafond est ailleurs** : **67 ancrages sur 120 sont absents du top-10 du reranker** — ce jeu borne d'abord la RÉCUPÉRATION, non mesurée pour l'instant. **Les deux comptes ont deux signes** : à k=10, 58 par identifiant contre 61 par texte (+3, dont G-112 et G-121 côté réglage, exactement les deux du §4.67 b) ; à k=1, 7 ancrages ont leur marqueur au prompt avec moins de 80 % de leur texte, parce que la fenêtre tronque. **Deux limites écrites dans le fichier** : la circularité est **déplacée, pas supprimée** (30 ancrages sur 120 au rang 1, et le juge de non-suffisance est le modèle qui a écrit la question) ; le jeu est **monolingue anglais**, toutes les questions françaises ayant été rejetées par le garde de vocabulaire. Banc en `cpu`, service en `cuda`. **MESURÉ PAR LE PILOTE SUR LE RÉSULTAT DE LA FUSION, CLONE PRINCIPAL** : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1160** ; mutations du **producteur** — `question_complete` en `any` au lieu de `all` → **1 rouge**, `…complete_que_si_tous_ses_ancrages_arrivent[identifiant-un seul arrive]` ; `rang_de` indexé à 0 → **1 rouge**, `…rang_est_1_indexe…` ; **le jeu** avec le second ancrage d'une question dupliqué → **2 rouges** (ancrages distincts, sections différentes) ; **témoin inerte** 13 lignes → `rc=0`, **32** sur le fichier du jeu ; SHA-256 changé à chaque mutation, **restaurations IDENTIQUES**. Une citation du corpus nomme des modèles commerciaux dans un champ `preuve` : c'est une donnée du corpus, pas une attribution. |
| **89** | LOT-37 — le plafond de récupération | **LIVRÉ et FUSIONNÉ le 25 septembre 2026** `ca1571a`, **1193 passés** (`tests/unit/`). `src/` N'EST PAS TOUCHÉ, vérifié — le §4.18 ne s'applique pas. **Le plafond n'est ni l'index, ni la profondeur seule, ni le reranker : c'est la REQUÊTE.** Contrôle positif d'abord : à profondeur de production, le banc retrouve **56 / 46 / 67** absents (dense, fusion, rerank), les chiffres du §4.76 à l'unité, et les **120 triplets de rangs identiques un à un** — une intersection, pas un accord de comptes. **H3 réfutée** : 109/109 ancrages indexés, 0 émietté, 109/109 portent leur preuve, trois détecteurs doublés par des témoins qui voient. **H2 réelle et bornée** : à seuil constant (top-10), **53 → 64 → 62** ancrages de 50 à 200 à 1000. **L'oracle `preuve` ramène 120/120** : aucun ancrage n'est hors d'atteinte du classement. **La décomposition de la question en sous-questions : 86 ancrages et 28 questions complètes sur 60, contre 53 et 7** — **borne supérieure**, l'affectation étant un oracle et la fusion réelle des deux sous-requêtes n'étant pas mesurée. Tableau des causes, qui somme à 120 : arrive **53**, écarté par le reranker **21**, profondeur qui récupère **13**, profondeur insuffisante **31**, requête unique **2**, hors ChromaDB **0**, texte indexé **0**, non expliqué **0**. **RECOUPÉ PAR LE PILOTE DEPUIS LES 120 OBSERVATIONS BRUTES** (`runs/2026-09-24-recuperation-causes.json`), par un classeur réécrit de ses mains : **mêmes comptes, 0 désaccord** avec la colonne `cause` ; questions complètes **7** en production et **28** en décomposition ; ancrages **86** en décomposition et **120** sous l'oracle `preuve`. **MESURÉ PAR LE PILOTE SUR LE RÉSULTAT DE LA FUSION, CLONE PRINCIPAL** : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1193** ; mutations du **producteur** — seuil du « haut » ignoré → **2 rouges** ; seuil strict (`<` au lieu de `<=`) → **2 rouges** ; marche H4 retirée du classeur → **3 rouges** ; **témoin inerte** 13 lignes → `rc=0`, **33** ; SHA-256 changé à chaque mutation, **restaurations IDENTIQUES**. **Le lot s'est pris trois fois lui-même et l'a écrit** : un classeur qui lisait « rendu par rerank » pour « dans le top-10 », trois mutations vides, et un compte de passes à 0 par `-qq`. **Trois questions ouvertes au §4.77** : la décomposition (+29 ancrages / +22 questions, borne haute), la profondeur 50 → 200 (+11, bornée), le reranker face à une question à deux besoins (52). **Aucun coût n'est mesuré, et aucune réponse n'est jugée.** |
| **—** | L'ATTESTATION D'AVANT LA PURGE *(geste du PILOTE, aucune conversation)* | **`mesuré` le 25 septembre 2026 à 00:32–00:33 UTC**, code servi `13acdb1` (`src/` identique à `main` : `git diff 13acdb1 HEAD -- src/` rend 0 ligne), avant la campagne de purge et de réingestion du pipeline. **Ancrages** : `make verifier-les-ancrages` → `rc=0`, **0 désaccord** ; 130/130, 44/44 et 109/109 présents dans le graphe ET dans ChromaDB, soit **267 ancrages distincts** sur l'union des trois jeux. **Clés médias** : **212**, SHA-256 de la liste triée **`c91f5be6e24fbcba…`**, identique à la référence du 22 septembre. **Graphe** : **23** `Document`, **15 173** arêtes `PARENT_OF` distinctes par `GO 1 TO 12 STEPS`. Ce sont les trois grandeurs que le pipeline s'est engagé à rendre identiques après réingestion. |
| **—** | LE REDÉPLOIEMENT D'AVANT LA CAMPAGNE, ET LE FEU VERT *(geste du PILOTE, aucune conversation)* | **Fait le 25 septembre 2026 à 00:36 UTC**, marche du §4 de `identite_du_code_servi.md`. Image servie `sha256:d296f489…` étiquetée **`rag-agent-chat-agent-api:2026-09-25-avant-campagne`** AVANT construction, étiquette vérifiée identique ; `make image` → `rc=0` ; `up -d --no-build agent-api` → `healthy` en 14 s, `RestartCount=0`, image `sha256:d15f8640…` ; `/health` publie `etat: identifie`, sha **`97bba20`** ; `POST /reindex` → 200, quatre sondes vertes. `src/` était déjà identique (0 ligne depuis `13acdb1`) : ce redéploiement sert à ce que le code servi porte le sha de `main` au moment de la purge. **Feu vert donné au pipeline** (message 32 de la série alternée) ; campagne démarrée côté pipeline (message 33, leur `main` = `d22a153`). **Aucune mesure contre les stores jusqu'à leur message de fin de réingestion.** |
| **90** | LOT-38 — la décomposition réelle de la question, et ce qu'elle coûte | **LIVRÉ et FUSIONNÉ le 25 septembre 2026** `5e0fc29`, **1224 passés** (`tests/unit/`). `src/` N'EST PAS TOUCHÉ, vérifié — le §4.18 ne s'applique pas. Le banc appelle les fonctions de `src/` sans les modifier : le modèle décompose la question **sans voir les passages ni le nombre de besoins** (3 sous-questions au plus, quatre natures de repli), chaque sous-question passe par la récupération de production, les listes sont fondues par le RRF de `src/`, puis le reranker score contre la question entière OU par sous-question au **maximum**. **Contrôle positif** : 53 ancrages / 7 questions sur le jeu dispersé, **120 rangs identiques un à un** au §4.77, et le banc refuse de publier sans cet accord. **Jeu dispersé** : production **7** questions complètes sur 60, fusion + rerank par sous-question **19**, borne de l'oracle **28** — **68 % de la borne**, et c'est le reranking par sous-question qui décide (+11 à liste fusionnée identique). **Non-régression** : réglage **127** en production contre **124** (perdues `G-097`, `G-110`, `G-119`, dont **2 imputables au retrait de la traduction** et non à la décomposition) ; contrôle **14 → 14**, un total immobile qui cache une perte (`q18`) et un gain (`q05`). **Contre la production, le bilan est +12 sur le jeu visé et −3 hors de lui.** **Coûts** : décomposition médiane **~0,7-1 s**, p95 **~1,5-1,7 s** sur le serveur réellement en production ; récupération des sous-questions **gratuite** (120 ms contre 130) ; reranking par sous-question **×1,6 à ×2** paires, p95 **8,5 s en cpu** (service en `cuda`, non mesuré). **LA CAMPAGNE DU PIPELINE A TRAVERSÉ LE LOT, ET LE LOT L'A VUE** : `chromadb: false` à 03:43, 4127 chunks à 03:48, 4290 à 03:50 ; aucune mesure dans cette fenêtre, ancrages revérifiés à 03:53 (`rc=0`, 0 désaccord), puis **4367 → 4367** aux deux bouts de chaque campagne. **RECOUPÉ PAR LE PILOTE DEPUIS LES LIGNES BRUTES** des trois jeux (`runs/2026-09-25-fusion-*.json`) : les quatre variantes rendent exactement **(53, 7), (57, 6), (60, 8), (72, 19)** sur le jeu dispersé, **127 / 125 / 124 / 124** sur le réglage et **14** partout sur le contrôle ; perdues contre la base appariée : `D-003`, `G-110`, `q18`, recalculées par différence d'ensembles. **MESURÉ PAR LE PILOTE SUR LE RÉSULTAT DE LA FUSION, CLONE PRINCIPAL** : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1224** ; mutations du **producteur** — `min` au lieu de `max` dans le rerank par sous-question → **1 rouge** ; une sous-question unique traitée comme une décomposition → **2 rouges** ; `any` au lieu de `all` pour la quasi-identité → **1 rouge** ; **témoin inerte** 13 lignes → `rc=0`, **31** ; SHA-256 changé à chaque mutation, **restaurations IDENTIQUES**. **Non mesuré** : la décomposition AVEC traduction, la variante « moyenne », le coût du reranker en `cuda`, la répartition des 9 questions d'écart entre fusion et reranking, et aucune réponse n'est jugée. |
| **—** | LE VERDICT D'APRÈS LA CAMPAGNE DU PIPELINE *(geste du PILOTE, aucune conversation)* | **`mesuré` le 25 septembre 2026 à 06:17 UTC**, code servi `97bba20`, après la purge et la réingestion du pipeline (observées par le lot 38 entre 03:43 et 03:53 UTC). **TOUT EST IDENTIQUE À L'ATTESTATION D'AVANT** : `POST /reindex` → `{"chunks_indexed":4367}` ; `make verifier-les-ancrages` → `rc=0`, **0 désaccord**, 130/130, 44/44 et 109/109 dans le graphe ET dans ChromaDB, soit les **267** ancrages ; **212** clés médias, SHA-256 **`c91f5be6e24fbcba…`**, identique à la référence du 22 septembre ; **23** `Document` et **15 173** arêtes `PARENT_OF` distinctes. Ce relevé est fait AVANT le message de fin du pipeline : il atteste un état stable à 06:17, pas la clôture de leur campagne. **Ajouté le même jour, à réception du message 34** : le pipeline date la fin de sa réingestion à **03:50:41 UTC** (23 runs `SUCCESS`, `comparer` 23/23, **0 `element_id` déplacé**), donc ce relevé de 06:17 est postérieur à la clôture et vaut verdict. Verdict rendu au message 35 : **conforme sur les cinq contrôles**. |
| **91** | LOT-39 — la décomposition avec traduction, et l'écart à la borne réparti par étage | **LIVRÉ et FUSIONNÉ le 25 septembre 2026** `618730c`, **1263 passés** (`tests/unit/`). `src/` N'EST PAS TOUCHÉ, vérifié. **Ce prompt avait été ANNULÉ par le pilote le matin même au profit du lot de documentation, et il a été lancé quand même** ; son résultat est une mesure sans risque, il est fusionné, et la documentation passe au lot suivant. Trois variantes neuves, les décompositions du §4.78 **relues et non refabriquées**, la traduction prise dans `src/`. **Contrôle positif sur trois termes** : les douze couples du §4.78, les rangs de ses quatre variantes un à un (**297 clés, 0 désaccord**), les 120 rangs du §4.77. **Questions complètes** — dispersé : production **7**, `fusion_rerank_sous_questions` **19**, traduite **18**, question traduite puis décomposée **19** ; réglage : **127 / 124 / 125 / 127** ; contrôle **14** partout. **La traduction ne rend rien sur le jeu qu'elle vise** (monolingue anglais), mais elle ferme presque toute la perte du jeu de réglage (**127**, dont un gain reposant sur une panne du producteur, `G-024`, et c'est dit). **Diagnostic des perdues** : `G-097` perdue par le REPLI qui abandonnait la traduction ; `G-119` perdue à la FUSION (rang 49 d'une sous-requête, coupé) — l'imputation du §4.78 était fausse ; `G-110` perdue par la décomposition ; `q18` perdue au RERANKING alors qu'elle était première de la liste fusionnée. **L'écart à la borne n'est pas 28 − 19 = 9** : c'est **11 manquées et 2 gagnées hors borne**, dont **6 à la fusion**, 4 au reranking, 1 jamais récupérée. **RECOUPÉ PAR LE PILOTE DEPUIS LES LIGNES BRUTES** des trois jeux : les sept variantes rendent exactement les comptes publiés. **MESURÉ PAR LE PILOTE SUR LE RÉSULTAT DE LA FUSION, CLONE PRINCIPAL** : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1263** ; mutations du **producteur** — l'étage « perdu au reranking » retiré du classeur → **5 rouges** ; l'ordre des étages inversé (aval avant amont) → **1 rouge** ; **témoin inerte** 13 lignes → `rc=0`, **39** ; restaurations IDENTIQUES au SHA-256. Aucune réponse jugée, aucune recommandation. |
| **92** | LOT-40 — la documentation du livrable | **LIVRÉ et FUSIONNÉ le 25 septembre 2026** `62003e9`, **1263 passés** (inchangé, aucun test touché). Cinq fichiers de documentation, `src/`, `tests/` et `Makefile` non touchés, vérifié : `README.md` réécrit pour un lecteur neuf ; **`documentation/etat_du_projet.md`** (neuf) — ce qui marche avec ses mesures, ce qui ne marche pas, les limites, chaque chiffre repris de son site canonique (§4.66 à §4.79, lignes 79 à 91) ; **`documentation/prochaines_etapes.md`** (neuf) — les questions ouvertes ordonnées par le coût de l'échec, efforts étiquetés `supposé` ; un bandeau daté sur `rag_evaluation_strategy.md` (deux jeux → trois) et une correction datée sur `llm.md` (fenêtre servie 32768, défaut 8192), rien effacé. Les onze routes de l'API relevées deux fois indépendamment (`/openapi.json` servi et `src/api/main.py`), listes identiques. Chaque commande du README est exécutée ou marquée « non exécutée ici » avec sa raison. **Le lot s'est repris lui-même** sur les « 37 pertes sèches » que le §4.71 avait retirées. **VÉRIFIÉ PAR LE PILOTE** : aucune mention d'outil, aucune adresse IP interne, aucune valeur de secret dans le diff ; chiffres clés de l'état du projet recoupés avec les §4.76 à §4.79 ; porte sur la fusion `rc_lint(make)=0` / `rc_test(make)=0`, **1263**. **Non vérifié** : les licences reprises de l'ancien README (seule celle du SDK `minio` l'est), et les 62 mentions du produit de stockage qui deviendront fausses après la bascule SeaweedFS. |
| **—** | LA BASCULE DU STOCKAGE OBJET, MinIO → SeaweedFS *(geste du PILOTE, aucune conversation)* | **Faite le 25 septembre 2026 à 09:00 UTC**, à réception du message 39 du pipeline (leur bascule : 08:15 UTC, 23 runs `SUCCESS`, `comparer` 23/23, 0 déplacé). **Changement de configuration seulement, aucune image reconstruite** : dans le `.env` du clone principal, `MINIO_ENDPOINT` passe à l'hôte SeaweedFS, et `MINIO_ROOT_USER` / `MINIO_ROOT_PASSWORD` prennent le jeu **LECTURE SEULE** du pipeline, recopié par script depuis leur `.env` sans jamais être affiché ; `MINIO_BUCKET` et `MINIO_SECURE` inchangés. **Chemin du retour** : l'ancien `.env` est copié, droits 600, HORS du dépôt, à `/home/ubuntu/RAG/rag-agent-chat.env.avant-bascule-seaweedfs` ; le recopier puis `docker compose up -d --no-build agent-api`. `up -d --no-build agent-api` → `healthy` en 13 s, `RestartCount=0`. **PREUVES QUE L'AGENT LIT SEAWEEDFS, ET NON L'ANCIEN MINIO RESTÉ DEBOUT** : le témoin `temoin-bascule/bascule-2026-09-25.txt`, qui n'existe que dans SeaweedFS, est lu par le client de l'agent — **219 octets, SHA-256 `39e06d1d…ae0`**, celui annoncé par le pipeline ; le journal porte `MinIO connecté : seaweedfs:8333`. **CONTRÔLES D'APRÈS, TOUS IDENTIQUES** : `GET /media/…/086f1173cb_picture.png` → **200**, **9 986 octets**, SHA-256 **`9cc9c4ed…`** égal aux octets lus AVANT dans MinIO ; `POST /reindex` → **4 367** chunks ; `make verifier-les-ancrages` → `rc=0`, **0 désaccord**, 267 ancrages ; **212** clés, empreinte **`c91f5be6e24fbcba…`** ; **23** `Document`, **15 173** arêtes. **ET UN DÉFAUT RÉEL TROUVÉ EN CHEMIN, §4.80** : avant la bascule, le proxy `/media` du service rendait **404 sur toutes les images** depuis la purge du pipeline, `/health` restant vert, parce que la session NebulaGraph du processus ne connaissait plus les tags. La recréation du conteneur l'a fermé pour cette fois ; **il reviendra à la prochaine purge**. |
| **93** | VÉRIF-40 — vérification indépendante de la documentation du livrable | **DEUX BLOQUANTS, FERMÉS PAR LE PILOTE LE 25 SEPTEMBRE 2026** `c5ce990` ; rapport `documentation/audits/2026-09-25-verification-lot-40.md`. Depuis un clone NEUF, suivi à la lettre : **B1** — la porte du README échouait (`rc=2`), faute d'une ligne d'activation du `.venv` ; **B2** — « `reviewed: false` sur les trois jeux, 138 + 30 + 60 » et « aucune question relue » étaient **faux** : **190 sur 228** ne sont pas relues, les **30** du jeu de contrôle et **8** du jeu de réglage le sont. L'erreur venait du registre lui-même (§4.78 point 7, §4.79 réserve 8, et la ligne P1 du tableau des priorités), **corrigé au site en ajoutant**. Sur **319** chiffres de `etat_du_projet.md` relevés par script : **317 identiques, 0 faux**, 1 arrondi non dit, 1 sans source. **Onze routes servies, onze décrites, identiques.** 0 adresse IP, 0 hôte de stockage, 0 secret. **FERMÉ PAR LE PILOTE, ET LA DÉROGATION EST ÉCRITE ICI** : quatre fichiers de documentation corrigés de ses mains plutôt qu'en conversation de réparation, parce que c'est le jour du livrable et que chaque correction est une phrase. Les non bloquantes N1 (le message d'échec annoncé), N2 (« 8,5 s » pour 8 483 ms), N4 (« refuse » pour « ne recommande pas ») et la durée sans source sont corrigées au même commit. **PREUVE DE LA FERMETURE DE B1** : clone neuf de `c5ce990` depuis le dépôt distant, commandes du README à la lettre, `PATH` minimal sans `.venv` hérité — installation `rc=0`, puis `. .venv/bin/activate && make lint && make test` → `rc_lint(make)=0` / `rc_test(make)=0`, **1263 passés** en 131 s, 09:19–09:22 UTC. |
| **94** | LOT-41 — après une purge du graphe, l'agent rouvre sa session | **LIVRÉ, FUSIONNÉ ET SERVI le 25 septembre 2026** `02f3213`, **1278 passés** (`tests/unit/`). Ferme le §4.80. **La forme de l'erreur n'était pas celle qu'on croyait** : `nebula3` ne lève pas, il rend un `ResultSet` en échec (`E_SEMANTIC_ERROR`, `-1009`), donc la reprise existante, posée dans un `except`, ne voyait rien. **Et le défaut touchait aussi la reconstruction** : `GO … OVER PARENT_OF` échoue de la même façon, la remontée au parent rendait `(None, 0)`, et `reconstruct_section` perdait son ancrage en silence ; `YIELD 1` réussissait, d'où un `/health` vert. Correctif : sur ce seul échec de schéma inconnu, rouvrir le pool et rejouer UNE fois ; la liste blanche du proxy ne retient jamais un vide ; la sonde `/health` lit un tag (`LIMIT 1`). **AUDIT INDÉPENDANT NON DEMANDÉ, ET LA RAISON EST ÉCRITE ICI** : jour du livrable, diff exécutable court et localisé, table des mutations fournie dans les deux sens, **remesurée par le pilote sur le résultat de la fusion**. **MESURÉ PAR LE PILOTE, clone principal, `.env` réel** : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1278** ; reprise retirée → **5 rouges** ; sonde de tag retirée → **2 rouges** ; cache du vide rétabli → **1 rouge** ; **témoin inerte** → `rc=0`, **24** ; restaurations IDENTIQUES au SHA-256. **UNE MUTATION SURVIT, ET C'EST UNE NON BLOQUANTE** : retirer la condition sur le CODE d'erreur (le message seul décidant) laisse les 24 verts — une erreur d'une autre nature portant le même libellé déclencherait une réouverture de trop, sans autre dommage. Non réparée. **À la fusion, le pilote a retiré du registre un chemin de répertoire temporaire qui portait le nom de l'outillage de session.** **REDÉPLOYÉ à 10:08 UTC** : image servie `sha256:d15f8640…` étiquetée `rag-agent-chat-agent-api:2026-09-25-avant-lot41` avant construction ; `/health` publie `02f3213`, quatre services verts, la sonde lisant désormais un tag ; `GET /media/…` → **200**, octets de SHA-256 `9cc9c4ed…` identiques ; une vraie question → **200**, 3 820 caractères, 14 citations, 29,9 s. **Non prouvé** : la fermeture sous une VRAIE purge — elle exigerait de purger le graphe partagé. |
| **—** | LE GEL DU LIVRABLE, `v1.0.0` *(geste du PILOTE, aucune conversation)* | **Posé le 25 septembre 2026 vers 10:11 UTC.** Étiquette git annotée **`v1.0.0`** sur `37182cc`, tagger `florian_horellou@laposte.net`, poussée (`refs/tags/v1.0.0`) ; `src/` identique au code servi `02f3213` (`git diff` : 0 ligne). Image servie `sha256:d24c2e4a…` étiquetée **`rag-agent-chat-agent-api:v1.0.0`**, vérifiée identique à celle du conteneur. Porte **1278**, CI verte sur `02f3213`. Stockage objet : SeaweedFS depuis 09:00 UTC. État et limites : `documentation/etat_du_projet.md` ; suite : `documentation/prochaines_etapes.md`. |
| **—** | LE MÉNAGE DE REPRISE *(geste du PILOTE, à la demande du propriétaire, aucune conversation)* | **Fait le 25 septembre 2026.** **1. Le nom de l'outillage de session sort du dépôt** : 68 lignes dans 15 fichiers de `documentation/` — chemins d'arbres de travail et préfixes de branche remplacés par `<arbre de travail>` et `<branche de session>`, motifs de détection cités remplacés par `<motifs d'attribution>`, et le plan historique `llm_integration_plan.md` neutralisé (nom de produit et clé d'API remplacés par des termes génériques). **LES RAPPORTS D'AUDIT ONT ÉTÉ TOUCHÉS, CONTRE LA RÈGLE QUI LES DIT IMMUABLES, SUR DÉCISION DU PROPRIÉTAIRE** : la règle « aucune mention de l'outil, nulle part » prime ; seuls des noms de chemin et de branche ont changé, aucun constat ni aucun chiffre. L'exclusion du répertoire de l'outillage quitte `.gitignore` pour `.git/info/exclude`, local au poste, et le README dit de la poser sur un poste neuf ; `.dockerignore` exclut désormais tout fichier caché de la racine (`.*`), `Dockerfile.agent` n'en copiant aucun. **Restent, et c'est voulu** : les motifs que les garde-fous d'attribution DÉTECTENT (`scripts/git-hooks/`, `tests/unit/test_installation_des_garde_fous.py`), une citation du corpus dans `tests/fixtures/` et `runs/` (le texte d'un livre, qui doit rester identique au chunk), et l'avatar du rôle assistant dans l'interface (`src/frontend/app.py`). **2. Les traces du stockage d'avant** : `.env.example` et les défauts de `settings.py` pointaient `minio:9000`, l'ancien stockage RESTÉ DEBOUT — un poste neuf aurait lu l'ancien MinIO sans aucune erreur, exactement le piège du message 37 ; ils pointent désormais SeaweedFS, les clés par défaut sont vides, et `.env.example` dit d'y mettre le jeu lecture seule du pipeline. Cinq commentaires de `src/` suivent. **3. Le README gagne une section 0, « Reprendre le projet »** : quoi lire, où vivent les faits, monter le poste, configurer, vérifier les stores, déployer. **Contrôles de reprise** : 0 lien relatif mort dans les `.md` suivis (un faux positif, une expression régulière citée) ; `.env.example` et les alias de `settings.py` identiques dans les deux sens ; 0 script de `scripts/` jamais cité ; toutes les docs de `documentation/` présentes dans la carte du README. Porte `rc_lint(make)=0` / `rc_test(make)=0`, **1278**. |
| **—** | LE LIVRABLE SERVI, `v1.0.1` *(geste du PILOTE, aucune conversation)* | **25 septembre 2026, 10:27 UTC.** Redéployé après le ménage, marche du §4 : image servie `sha256:d24c2e4a…` étiquetée `rag-agent-chat-agent-api:2026-09-25-avant-menage` avant construction ; `/health` publie `identifie`, **`d4615e5`**, quatre services verts ; `GET /media/…` → 200 ; journal `MinIO connecté : seaweedfs:8333` ; le conteneur ne porte que `data`, `prompts` et `src`. **CI verte sur `d4615e5`** (run `36124019252`). Étiquette git annotée **`v1.0.1`** sur `d4615e5`, poussée ; image servie `sha256:cb8f15aa…` étiquetée **`rag-agent-chat-agent-api:v1.0.1`**. `v1.0.0` reste en place et n'est pas déplacée. |
| **—** | LES ÉMOJIS SORTENT DU DÉPÔT *(geste du PILOTE, à la demande du propriétaire, aucune conversation)* | **Fait le 25 septembre 2026.** Tous les pictogrammes des fichiers suivis sont retirés (20 fichiers ; les flèches et les triangles typographiques restent, ce sont des signes et non des émojis). Dans la documentation, `OK`, `NON`, `ATTENTION`, `ROUGE` et `oui` remplacent les pictogrammes équivalents, sans qu'aucun constat ni aucun chiffre ne change. Dans l'interface, les boutons et les titres perdent leur pictogramme, l'icône de page disparaît, et l'historique écrit `Vous :` et `Agent :`. **Les garde-fous d'attribution détectent toujours l'émoji de robot**, désormais construit par ses octets UTF-8 (`printf '\360\237\244\226'`) au lieu d'être écrit : éprouvé à la main — un message qui le porte est refusé, un message propre passe. Relevé après coup par script : **0** pictogramme dans les fichiers suivis. Porte `rc_lint(make)=0` / `rc_test(make)=0`, **1278**. |
| **—** | LE LIVRABLE SERVI, `v1.0.2` *(geste du PILOTE, aucune conversation)* | **25 septembre 2026, 10:44 UTC.** Agent ET frontend reconstruits et redéployés : images servies étiquetées `rag-agent-chat-agent-api:2026-09-25-avant-emojis` et `rag-agent-chat-frontend:2026-09-25-avant-emojis` avant construction, vérifiées identiques ; `/health` publie `identifie`, **`4175c60`**, quatre services verts ; `GET /media/…` → 200 ; frontend → 200 sur le port 8506, **0** pictogramme dans son `app.py` servi. CI verte sur `4175c60`. Étiquette git **`v1.0.2`** poussée ; images étiquetées `v1.0.2`. `v1.0.0` et `v1.0.1` restent en place. |
| **95** | LOT-42 — lire `media_url` et `object_key`, avec repli sur `minio_url` : l'étape 1 de la sortie de MinIO | **LIVRÉ, FUSIONNÉ ET SERVI le 25 septembre 2026** `3f7203f`, **1304 passés**. Le pipeline renomme `minio_url` en `media_url` et ajoute `object_key`, dans le graphe ET dans ChromaDB ; sa purge recrée le schéma, donc **deux états seulement**, jamais les deux champs ensemble (message 45). Chaque site de lecture lit `media_url`, puis `minio_url` à défaut, et `object_key` ; la clé d'objet vient d'`object_key` s'il est publié, sinon de l'URL ; la liste blanche passe par le même décodeur que le proxy, ce qui ferme les deux décodeurs divergents du §4.62. **Mesuré sur le graphd réel, en lecture seule** : une propriété ABSENTE du schéma d'un tag existant ne fait pas échouer la requête — `__NULL__` en `RETURN`, 0 ligne en `WHERE <absente> != ""`, et le `OR` tient dans les deux ordres ; la forme retenue rend **212** objets. Rien d'autre n'est renommé (étape 3). **AUDIT INDÉPENDANT NON DEMANDÉ**, pour la raison des lignes 87 et 94 : jour du livrable, pipeline en attente, table des mutations fournie (24 mutations, une survivante tuée par une scène ajoutée), remesurée par le pilote. **MESURÉ PAR LE PILOTE SUR LA FUSION, CLONE PRINCIPAL** : porte `rc_lint(make)=0` / `rc_test(make)=0`, **1304** ; `media_url` ignoré au site lexical → **6 rouges** ; repli `minio_url` retiré au sommet → **3 rouges** ; `media_url` retiré de la liste blanche → **2 rouges**, de la seule garde d'inventaire — les doubles n'exécutent pas le nGQL, et la preuve de ce site est donc sur le service après la réingestion ; témoin inerte → `rc=0`, **1304**. **SERVI à 13:35 UTC** : image étiquetée `2026-09-25-avant-lot42` avant construction ; `/health` publie `3f7203f`, quatre services verts ; `Proxy média : 212 objets autorisés.` ; une vraie question → 200 avec son image, et l'image → 200. **Message 46 envoyé au pipeline : servi.** |
| **—** | LE VERDICT D'APRÈS LA SORTIE DE MinIO DU PIPELINE *(geste du PILOTE, aucune conversation)* | **`mesuré` le 25 septembre 2026 à 14:22 UTC**, code servi `3f7203f`, après la purge, la réingestion et la suppression de l'ancien stockage objet côté pipeline (13:47–13:50 UTC, message 48). **CONFORME, ET L'AGENT LIT LE NOUVEAU CONTRAT** : `Proxy média : 212 objets autorisés.` sur un graphe qui ne porte plus que `media_url` et `object_key` ; `GET /media/…` → 200, octets `9cc9c4ed…` identiques ; empreinte des 212 clés `c91f5be6e24fbcba…` ; 23 `Document`, 15 173 arêtes ; `POST /reindex` → 4 367 chunks ; `make verifier-les-ancrages` → `rc=0`, 0 désaccord ; une vraie question → son image. **LE CORRECTIF DU LOT 41 N'A ÉTÉ ÉPROUVÉ QU'À MOITIÉ** : pendant la purge, le journal porte deux fois `Session NebulaGraph périmée (… Document': Unknown tag) : réouverture du pool` (13:38:07 et 13:38:27), puis l'échec du rejeu — le tag n'existait pas encore, donc `/health` rouge, ce qui est JUSTE pendant une purge. Mais le pipeline a **redémarré notre conteneur** à 13:39:27 (`docker restart`, contre notre demande, écart reconnu au message 48) : que la réouverture rende la main une fois le schéma recréé n'est donc **pas mesuré**. La prochaine purge devra le faire, sans redémarrage. |

**Prochain numéro libre : 96.**
