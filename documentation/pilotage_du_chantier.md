...
| **7** | le garde de sûreté ne voit ni `monkeypatch.setattr(settings, …)` (**102** sites) ni la lecture d'environnement à valeur par défaut (**6** sites) ; et la lecture défensive du reranker ne tient que dans **trois** natures d'exception sur six (§4.35) | à distribuer — **les deux demandent une décision, pas une réparation** |
