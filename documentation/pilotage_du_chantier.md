...
| **7** | le garde de sûreté ne voit ni `setattr(settings, …)` (**47** occurrences suivies) ni la lecture d'environnement à valeur par défaut (**6** sites suivis — le sixieme est un ALIAS, `env = os.environ.get`, invisible a un motif exigeant une parenthese collee) ; et la lecture défensive du reranker ne tient que dans **trois** natures d'exception sur sept sondées, **quatre** propageant et cassant la recherche (§4.35, mesuré au §4.37) | à distribuer — et **moins cher qu'annoncé** : `mesuré`, **aucun** des 27 `setattr` sur `embedding_model_name` ne passe un littéral, ils passent tous une constante nommée, donc l'obstacle invoqué pour le différer **n'existe pas** |
