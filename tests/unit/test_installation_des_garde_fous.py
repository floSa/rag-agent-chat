...
class TestLeScriptConstateSonPropreResultat:
    ...
    def test_un_installeur_qui_ecrase_le_hook_du_framework_est_refuse(self, tmp_path: Path):
        ...
        par `if false; then` rendait `rc=0` et **0 rouge** sur les 14 tests. Le
        defaut que cette moitie surveille — la copie manuelle passee APRES
        `pre-commit install`, qui ecrase le hook genere — etait bien vu, mais par
        DOUZE autres tests, jamais par elle : sur l'inversion complete des deux
