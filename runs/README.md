...
| `2026-09-10-ancien-lecteur-reglage.json` | 138 questions régénérées | **Le rejeu de la référence contre le lecteur du 3 septembre 2026, le jour de son remplacement.** `make eval` le 10 septembre 2026 de 13:47 à 14:05 UTC, depuis l'arbre du lot 8, contre l'image `946dc14c…` (construite le 3 septembre) — le code de `main` n'y était pas, ni le garde du modèle d'embedding. **Reproduit la référence à l'unité** sur les dix métriques appariées : 130/130 (138/138 pour les deux `↓`) ex æquo, `Δ = 0`, `p = 1`. Ne bougent que la génération (`citations_par_reponse` 3,217 → 3,109, `reponse_caracteres_p50` 607 → 570) et les latences — le LLM n'est pas déterministe, et cette campagne a partagé l'hôte avec `make lint` puis `make test` du même lot, donc ses latences ne valent rien. C'est la colonne « avant » de la reconstruction, et la preuve que l'index, les modèles et la recherche n'ont pas bougé sous les antécédents du 8 septembre. Renommé depuis `20260910-1347-reglage.json`, le nom que la recette écrit. |
| `2026-09-10-ancien-lecteur-controle.json` | 30 questions du pipeline | Même rejeu, `make eval-controle` le 10 septembre 2026 de 14:05 à 14:10 UTC, même image. **Reproduit le contrôle à l'unité** : 26/26 (30/30 pour les deux `↓`) ex æquo. Renommé depuis `20260910-1405-controle.json`. La réserve du jeu voyage avec lui. |
| `2026-09-10-lecteur-neuf-reglage.json` | 138 questions régénérées | **La colonne « après » de la reconstruction** : `make eval` le 10 septembre 2026 de 14:13 à 14:31 UTC contre l'image `2f4f1aa9…` construite depuis `main` = `85b5a7a` et mise en service à 14:13 UTC — le code des lots 2 à 7, les gardes du modèle d'embedding et du reranker enfin en production. **Reproduit la référence du 8 septembre à l'unité** sur les dix métriques appariées (130/130, 138/138 ex æquo, `Δ = 0`) : sept lots n'ont rien changé au contenu servi, et c'est ce qu'ils promettaient — des gardes et des instruments, pas un réglage. Latences non contaminées, à une sonde légère près (une lecture du reranker dans le conteneur à 14:14 UTC). Renommé depuis `20260910-1413-reglage.json`. Compte rendu : [`2026-09-10-reconstruction-du-lecteur.md`](../documentation/campagnes/2026-09-10-reconstruction-du-lecteur.md). |
| `2026-09-10-lecteur-neuf-controle.json` | 30 questions du pipeline | Même lecteur neuf, `make eval-controle` de 14:31 à 14:36 UTC : **26/26 et 30/30 ex æquo** avec le contrôle du 8 septembre. Renommé depuis `20260910-1431-controle.json`. La réserve du jeu voyage avec lui. |
| `2026-09-10-definition-c-allumee-reglage.json` | 138 questions régénérées | **La colonne « après » du lot 4, enfin versionnée** — le §4.42 relevait qu'elle n'avait aucun artefact. `scripts/evaluate.py` le 10 septembre 2026 de 14:39 à 14:58 UTC, `--compare runs/2026-09-08-reference.json`, contre un lecteur **HORS SERVICE** lancé sur l'hôte depuis l'arbre du lot 8 (`5a98e41`, `uvicorn`, port 8012, `NEIGHBOUR_SECTION_UNCLES=true`, mêmes stores, même LLM) : le conteneur en service n'a pas été touché. **Reproduit la campagne du lot 4 à l'unité sur ce qui décide** : `caracteres_retenus_p50` 9 529 → **10 633**, `prompt_eval_count_p50` 3 131 → **3 291**, `contextes_ecartes_total` 26 → **32**, les cinq métriques de rappel **130/130 ex æquo**. `reconstruction_ms` 114 → 191 p50 / 181 → 550 p95 (le lot 4 : 197 / 571) — latences d'un processus hôte, pas du conteneur, à lire comme un ordre de grandeur. Compte rendu : [`2026-09-10-reconstruction-du-lecteur.md`](../documentation/campagnes/2026-09-10-reconstruction-du-lecteur.md), §8. |
| `2026-09-10-definition-c-allumee-controle.json` | 30 questions du pipeline | Même lecteur hors service, 14:58 → 15:02 UTC, `--compare runs/2026-09-08-controle-30.json`. Rappel 26/26 ex æquo ; `contextes_ecartes_total` 8 → 11, `caracteres_retenus_p50` 9 651 → 10 916, `rappel_contexte` 0,712 → 0,731 (**une** question, q04 — du bruit au sens de la réserve du jeu). |
| `2026-09-15-ollama-reference-avant-vllm-reglage.json` | 138 questions régénérées | **LA RÉFÉRENCE OLLAMA D'AVANT LA BASCULE vLLM**, et la PREMIÈRE campagne du dépôt qui porte `moteur_llm` renseignée. `make eval` le 15 septembre 2026 de **21:23:57 à 21:56:02 UTC** (32 min 05 s), `rc` de `make` = 0, **138/138**, contre l'agent en service redéployé le même jour à 21:21:05 UTC sur l'image `b2f787a4…` construite depuis `main` = `8209e68`. Écrite par la recette sous le nom `20260915-2123-reglage.json`. **Le moteur est enfin nommé** : `ollama 0.30.10`, `gemma4:e4b@c6eb396dbd5992bb`, `Q4_K_M`, `num_ctx` 8192, `thinking` **false** — la comparaison annonce donc `MOTEUR LLM INCONNU D'UN CÔTÉ AU MOINS`, la référence du 8 septembre étant **MUETTE**. C'est attendu : `muet` n'est pas `différent`, et aucune campagne antérieure ne pouvait porter la clé. **Le rappel est intact** : `rappel_recherche` 0,962, `rappel_elements` 0,954, `rappel_documents` 1,0, `taux_citation_complete` 1,0, `abstention_correcte` 1,0, `contextes_ecartes_total` 26 — tous ex æquo. Seul `rang_reciproque` bouge d'**une** question sur 130 (`mrr` 0,942 → 0,938, IC 95 % [−0,011, +0,000], `p` = 1,000) : non significatif. **LES LATENCES p95 NE DÉCRIVENT PAS NOTRE AGENT, ET NE DOIVENT PAS ÊTRE LUES COMME TELLES.** Un autre projet a disputé `ollama-central` pendant toute la campagne : `llama-server` a été relancé **onze fois** entre 21:23:50 et 21:33:27 UTC (PID 89329 → 3337861 → 3360269 → 3365265 → 3367629 → 3372221 → 3374756 → 3377558 → 3379319 → 3380462 → 3383005), et le modèle a **entièrement disparu** d'`/api/ps` à 21:28:54, 21:31:26 et 21:32:26. D'où `translation_ms_p95` 1 602 → **19 844** et `total_ms_p95` 29 166 → **45 032**. Le p50, lui, s'AMÉLIORE — `total_ms_p50` 7 298 → **6 819** —, et les étages GPU sont francs : `rerank_ms_p50` 498 → **58**, `retrieval_ms_p50` 700 → **229**, `dense_ms_p50` 120 → **76**. Le digest servi est resté `c6eb396dbd5992bb` sur **64 échantillons** à 30 s de pas malgré les onze relances : l'empreinte publiée, relevée à 21:21:16 UTC et mémorisée pour la vie du processus, est donc restée exacte toute la campagne. |
| `2026-09-23-selection-auto-select-top-k.json` | 130 questions à ancrage du jeu de réglage | **LE QUATRIÈME ÉTAGE DE LA CHAÎNE, ET CE N'EST PAS UNE CAMPAGNE** : aucune réponse n'y est générée. `scripts/mesurer_selection.py` le 23 septembre 2026 de 08:01 à 08:07 UTC, base `main` = `cec564a`, neuf valeurs d'`AUTO_SELECT_TOP_K` (1 à 20) rejouées sur le même retrieval. **Le rappel au prompt plafonne à 0,9538 dès k=2 et ne bouge plus** : passer de 3 à 6, ou de 3 à 10, ne gagne **aucune** question. La raison est dans le jeu, pas dans la chaîne — **aucun de ses ancrages ne se trouve entre les rangs 3 et 10** du reranker, la question ayant été écrite POUR son passage. Ce fichier mesure donc la RÉCUPÉRATION une fois de plus, et il est **aveugle à la sélection** : ne le cite pas comme un verdict sur k. `questions_avec_section_ecartee` vaut **0** aux neuf valeurs. Constat : [`axes_amelioration.md`](../documentation/axes_amelioration.md) §4.67. |
| `2026-09-23-selection-controle-30.json` | 26 questions à ancrage du jeu du pipeline, **47 ancrages** | **LE SEUL DES DEUX QUI TESTE VRAIMENT k**, parce que ses questions portent jusqu'à **trois** ancrages dispersés — le jeu de réglage n'en porte qu'un. Même banc, 08:07 à 08:09 UTC. Le rappel monte continûment : **0,6154 à k=1 → 0,7308 à k=3 → 0,7692 à k=6 → 0,8077 à k=10**, ancrages **18 → 26 → 28 → 33 sur 47**, **0 question perdue** à chaque transition. **ET IL NE TRANCHE RIEN** : sa réserve voyage avec lui et elle n'est pas négociable — de 3 à 6 il y a **une** question sur 26, soit 3,8 points, du bruit au sens de cette réserve, et les intervalles de Wilson se recouvrent. |
...
> **IL A ÉTÉ TROUVÉ PÉRIMÉ LE 23 SEPTEMBRE 2026, ET IL AVAIT LA BONNE TAILLE.**
> Il portait **130** entrées pour un jeu de **130** questions, et l'intersection
> exacte de ses clés avec `golden_qa_generated.yaml` valait **ZÉRO** — avec
> `jeu_de_questions_pipeline.yaml` aussi. Il venait d'un jeu antérieur. Un
> balayage qui l'aurait cru couvrant aurait joué **toutes** ses questions sans
> traduction et aurait conclu, sans qu'aucune erreur n'apparaisse. Il porte
> désormais **286** entrées et couvre les deux jeux. **Un compte d'entrées ne
> dit pas qu'un cache couvre un jeu : seule l'intersection des clés le dit**,
> et c'est ce que `mesurer_selection.py` exige avant de mesurer quoi que ce soit.
