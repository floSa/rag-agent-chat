...
| `TORCH_DEVICE` | `cpu` | Périphérique de l'embedder et du cross-encoder. **`cpu` par défaut alors que l'image porte un build CUDA depuis le 11 septembre 2026, et c'est délibéré** : trois conditions indépendantes décident qu'un calcul part sur la carte — le build, la réservation du GPU au conteneur, ce réglage — et chacune suffit à tout ramener sur le CPU **sans rien dire**. `/health` publie les trois sous `torch_device`. Avant de basculer, lire le §7 de [gpu_cuda.md](documentation/gpu_cuda.md), qui chiffre ce que ça rapporte (11 % du temps au plus) contre ce que ça risque (la contention avec Ollama, qui porte 84 %). |
